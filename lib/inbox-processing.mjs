...
function buildProcessedSources(instanceRoot, processedPaths, explicitSources = []) {
  ...
  for (const value of processedPaths || []) {
    const pathHint = typeof value === 'string'
      ? portableProcessedPath(instanceRoot, value)
      : portableProcessedPath(instanceRoot, value?.path);
...
function findInboxConflictPaths(instanceRoot) {
  ...
  const conflicts = [];
  function walk(current, inheritedConflict = false) {
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(absolute, isConflict);
    }
  }
...
function heartbeatInboxProcessingUnlocked(instanceRoot, options = {}) {
  ...
  const now = owned.now;
  const leaseMs = Number(owned.lock.lease_duration_ms);
  const expiresAt = new Date(now.getTime() + leaseMs).toISOString();
  const updated = {
    ...owned.lock,
...
function buildReceipt(instanceRoot, lock, options, now, override = null) {
  ...
  return {
    receipt: {
      ...
      processed: processedPaths,
      unresolved_paths: claimedPaths.filter((item) => !processedPaths.includes(item)),
      processed_sources: buildProcessedSources(
        instanceRoot,
        processedPaths,
