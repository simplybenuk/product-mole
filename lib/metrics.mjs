...
function writeProcessedInboxItems(instanceRoot, processedPaths, options = {}) {
  ...
  const normalizedPaths = normalizeProcessedPaths(processedPaths, instanceRoot);
...
export function recordProcessedInboxItems(instanceRoot, processedPaths, options = {}) {
  ...
  const requestedPaths = normalizeProcessedPaths(processedPaths, instanceRoot);
  const receiptPaths = normalizeProcessedPaths(receipt.processed, instanceRoot);
  if (!isPathSubset(requestedPaths, receiptPaths)) {
    throw new Error(
  ...
  return writeProcessedInboxItems(instanceRoot, requestedPaths, { ...options, now });
}
