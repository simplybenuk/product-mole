...
/**
 * Normalize both the historical path-only shape and the ID-bearing receipt
 * shape. Source IDs are the dedupe key whenever present; paths remain on the
 * entry for navigation and for compatibility with old aggregate consumers.
 */
...
function findProcessedIdentity(index, entry) {
  ...
  if (sourceId && index.aliases.has(sourceId)) {
    return index.aliases.get(sourceId);
  }
  ...
  // A path can be reused by a different source. Only let a path bridge the
  // compatibility gap when one side is still legacy path-only data.
...
export function backfillProcessedInboxMetrics(instanceRoot, options = {}) {
  ...
  for (const receiptFile of receiptFiles) {
    ...
    const processed = normalizeProcessedEntries(processedItems, instanceRoot);
    ...
    for (const entry of processed) {
      addProcessedIdentity(seenForDay, entry);
    }
