...
const SOURCE_REF_PATH_PATTERN = /^\s*(?:-\s+)?path:\s*(["']?)([^"'\s#]+)\1\s*$/gm;
...
function collectStructuredReferences(value, location = '$', output = []) {
  ...
  if (typeof value.path === 'string' && !isSourceId(value.source_id)) {
    output.push({ path: value.path, kind: 'structured', location, object: value });
  }
  if (typeof value.current_path === 'string' && !isSourceId(value.source_id)) {
    output.push({ path: value.current_path, kind: 'structured', location, object: value });
  }
...
function candidateForReference(root, legacyPath, records) {
  ...
  for (const record of records) {
    ...
    if (record.content_hash && record.current_path) {
      ...
      if (resolved.status === 'resolved' && resolved.path && resolved.path !== pathInfo.relative) {
...
function discoverReferences(root, options = {}) {
  ...
  for (const file of files) {
    ...
    if (file.relative.endsWith('.yaml') || file.relative.endsWith('.yml')) {
      for (const match of text.matchAll(SOURCE_REF_PATH_PATTERN)) {
        const pathValue = match[2];
        if (!references.some((item) => item.artifact_path === file.relative && item.path === pathValue && item.kind === 'structured')) {
          references.push({ artifact_path: file.relative, path: pathValue, kind: 'yaml-structured', offset: match.index });
        }
      }
    }
  }
...
function replaceJsonReferences(value, resolvedByPath, changes, location = '$') {
  ...
  if (typeof value.path === 'string' && !value.source_id && resolvedByPath.has(value.path)) {
    value.source_id = resolvedByPath.get(value.path);
    changes.push({ location, path: value.path, source_id: value.source_id });
  }
  for (const [key, child] of Object.entries(value)) replaceJsonReferences(child, resolvedByPath, changes, `${location}.${key}`);
}
...
function applyYamlReferenceChanges(before, resolvedByPath, changes) {
  ...
  for (const [legacyPath, sourceId] of resolvedByPath) {
    for (let index = 0; index < lines.length; index += 1) {
      ...
      changes.push({ path: legacyPath, source_id: sourceId });
      index += 1;
    }
  }
  return lines.join('\n');
