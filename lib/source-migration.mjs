  ...
  return fs.readdirSync(recordsDir)
    ...
    .map((name) => {
      try {
        ...
        if (!record || !isSourceId(record.source_id)) return null;
        try {
          return validateSourceRecord(record, { instanceRoot: root }).valid ? record : null;
        } catch {
          return null;
        }
      } catch {
...
function candidateForReference(root, legacyPath, records) {
  ...
  for (const record of records) {
    const historicalPaths = [
      ...
      record.current_path,
      ...(Array.isArray(record.path_history) ? record.path_history : [])
...
function stageMigrationWrite(filePath, content) {
  ...
  try {
    ...
    if (stat.isFile()) originalMode = stat.mode & 0o7777;
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  try {
    fs.writeFileSync(temporary, content, 'utf8');
    if (originalMode !== null) fs.chmodSync(temporary, originalMode);
  } catch (error) {
    try {
