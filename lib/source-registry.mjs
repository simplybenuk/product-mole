...
export function findRegistryFindings(instanceRoot = process.cwd(), options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (parent.parent_source_id) {
      ...
      if (!parentEntries.length || (candidatesWithAttachmentArrays.length > 0 && !candidatesWithAttachmentArrays.some((candidate) => candidate.record.attachments.some((item) => item.source_id === parent.source_id)))) {
        findings.push(finding('attachment_link_conflict', {
          source_id: parent.source_id,
