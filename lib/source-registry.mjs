...
export function readSourceRecord(instanceRoot, sourceId) {
  ...
  if (result.record.source_id !== sourceId) {
    throw new SourceRegistryError(`Source record filename conflicts with embedded source ID: ${filePath}`, 'RECORD_FILENAME_CONFLICT', {
      path: filePath,
      requested_source_id: sourceId,
      embedded_source_id: result.record.source_id,
      finding_type: 'record_filename_conflict'
    });
  }
...
function sourceRoots(root, options = {}) {
  ...
  return configured?.length ? configured : DEFAULT_SOURCE_ROOTS;
}
...
export function findRegistryFindings(instanceRoot = process.cwd(), options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (Array.isArray(parent.attachments)) {
      for (const attachment of parent.attachments) {
        if (!asObject(attachment)
          || !isSourceId(attachment.source_id)
          || typeof attachment.relationship !== 'string'
          || !attachment.relationship.trim()
          || typeof attachment.label !== 'string'
          || !attachment.label.trim()) continue;
    ...
    if (parent.parent_source_id) {
      ...
      const candidatesWithAttachmentArrays = parentEntries.filter((candidate) => Array.isArray(candidate.record.attachments));
      if (!parentEntries.length || (candidatesWithAttachmentArrays.length > 0 && !candidatesWithAttachmentArrays.some((candidate) => candidate.record.attachments.some((item) => asObject(item) && item.source_id === parent.source_id)))) {
        findings.push(finding('attachment_link_conflict', {
          source_id: parent.source_id,
...
export function resolveSource(instanceRoot, sourceId, options = {}) {
  ...
  if (findings.some((item) => item.type === 'path_claim_conflict'
    || item.type === 'duplicate_record'
    || item.type === 'revision_conflict'
    || item.type === 'record_filename_conflict'
    || item.type === 'invalid_record')) status = 'conflict';
