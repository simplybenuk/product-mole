...
describe('source migration classification', () => {
  ...
  it('keeps malformed path-history records out of migration matching', () => {
    withWorkspace((root) => {
      ...
      const match = report.references.find((item) => item.legacy_path === '6-raw/inbox/known.md');
      assert.equal(match.classification, 'unresolved');
...
describe('source migration apply mode', () => {
  ...
  it('preserves restrictive artifact permissions during apply', () => {
    withWorkspace((root) => {
      const sourcePath = path.join(root, '6-raw/inbox/known.md');
      fs.writeFileSync(sourcePath, 'known', 'utf8');
      const source = registerSource(root, { path: sourcePath, sourceType: 'note' });
      const artifactPath = path.join(root, '4-context', 'restricted.json');
      fs.writeFileSync(artifactPath, `${JSON.stringify({ source_refs: [{ path: '6-raw/inbox/known.md' }] }, null, 2)}\n`, 'utf8');
      fs.chmodSync(artifactPath, 0o600);

      const applied = migrateSourceReferences(root, { apply: true });
      const mode = fs.statSync(artifactPath).mode & 0o7777;
      assert.equal(applied.counts.changed, 1);
  ...
  it('validates the report destination before applying staged artifact changes', () => {
    withWorkspace((root) => {
      const sourcePath = path.join(root, '6-raw/inbox/known.md');
      fs.writeFileSync(sourcePath, 'known', 'utf8');
      registerSource(root, { path: sourcePath, sourceType: 'note' });
      ...
      const before = `${JSON.stringify({ source_refs: [{ path: '6-raw/inbox/known.md' }] }, null, 2)}\n`;
      fs.writeFileSync(artifactPath, before, 'utf8');
      const reportPath = path.join(root, 'governance', 'run-receipts', 'source-migration', 'report');
      fs.mkdirSync(reportPath, { recursive: true });

      assert.throws(
        () => migrateSourceReferences(root, { apply: true, reportPath }),
        /Migration report path must be a file\./
      );
      assert.equal(fs.readFileSync(artifactPath, 'utf8'), before);
      assert.equal(fs.readdirSync(reportPath).length, 0);
    });
  });

  it('rolls back earlier artifact replacements when a later commit fails', () => {
    withWorkspace((root) => {
      const firstSourcePath = path.join(root, '6-raw/inbox/first.md');
      const secondSourcePath = path.join(root, '6-raw/inbox/second.md');
      fs.writeFileSync(firstSourcePath, 'first', 'utf8');
      fs.writeFileSync(secondSourcePath, 'second', 'utf8');
      registerSource(root, { path: firstSourcePath, sourceType: 'note' });
      registerSource(root, { path: secondSourcePath, sourceType: 'note' });

      const firstArtifactPath = path.join(root, '4-context', 'a.json');
      const secondArtifactPath = path.join(root, '4-context', 'b.json');
      const firstBefore = `${JSON.stringify({ source_refs: [{ path: '6-raw/inbox/first.md' }] }, null, 2)}\n`;
      const secondBefore = `${JSON.stringify({ source_refs: [{ path: '6-raw/inbox/second.md' }] }, null, 2)}\n`;
      fs.writeFileSync(firstArtifactPath, firstBefore, 'utf8');
      fs.writeFileSync(secondArtifactPath, secondBefore, 'utf8');

      const originalRenameSync = fs.renameSync;
      let renameCount = 0;
      fs.renameSync = (...args) => {
        renameCount += 1;
        if (renameCount === 4) throw new Error('simulated later write failure');
        return originalRenameSync(...args);
      };
      try {
        assert.throws(
          () => migrateSourceReferences(root, {
            apply: true,
            reportPath: 'governance/run-receipts/source-migration/report.json'
          }),
      ...
      assert.equal(fs.readFileSync(firstArtifactPath, 'utf8'), firstBefore);
      assert.equal(fs.readFileSync(secondArtifactPath, 'utf8'), secondBefore);
      assert.equal(
        fs.existsSync(path.join(root, 'governance', 'run-receipts', 'source-migration', 'report.json')),
        false
      );
      assert.deepEqual(
        fs.readdirSync(path.join(root, '4-context')).filter((name) => name.includes('.migration')),
        []
      );
    });
  });
  ...
  it('keeps registry-dependent scans recoverable when an attachment entry is null', () => {
    withWorkspace((root) => {
      const sourcePath = path.join(root, '6-raw/inbox/known.md');
      fs.writeFileSync(sourcePath, 'known', 'utf8');
      const source = registerSource(root, { path: sourcePath, sourceType: 'note' });
