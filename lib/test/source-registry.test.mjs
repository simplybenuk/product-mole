...
describe('source registry resolution lifecycle', () => {
  ...
  it('resolves a moved source in another supported workspace root', () => {
    withTempWorkspace((root) => {
      const originalPath = path.join(root, '4-context', 'source.md');
      const movedPath = path.join(root, '4-context', 'archive', 'source.md');
      fs.mkdirSync(path.dirname(originalPath), { recursive: true });
      fs.writeFileSync(originalPath, 'context source', 'utf8');
      const result = registerSource(root, { path: originalPath, sourceType: 'note' });
      fs.mkdirSync(path.dirname(movedPath), { recursive: true });
      fs.renameSync(originalPath, movedPath);

      const resolved = resolveSource(root, result.source_id);
      assert.equal(resolved.status, 'resolved');
      assert.equal(resolved.path, '4-context/archive/source.md');
      assert.equal(readSourceRecord(root, result.source_id).current_path, '4-context/source.md');
    });
  });

  it('treats a valid record under the wrong filename as a resolver conflict', () => {
    withTempWorkspace((root) => {
      const sourcePath = path.join(root, '6-raw', 'inbox', 'misnamed.md');
      fs.writeFileSync(sourcePath, 'misnamed record', 'utf8');
      const result = registerSource(root, { path: sourcePath, sourceType: 'note' });
      const canonicalPath = path.join(root, result.record_path);
      const misnamedPath = path.join(path.dirname(canonicalPath), 'wrong-name.json');
      fs.renameSync(canonicalPath, misnamedPath);

      const resolved = resolveSource(root, result.source_id);
      assert.equal(resolved.ok, false);
      assert.equal(resolved.status, 'conflict');
      assert.ok(resolved.findings.some((item) => item.type === 'record_filename_conflict'));
    });
  });
...
describe('source registry attachments and findings', () => {
  ...
  it('skips malformed attachment entries while retaining invalid-record findings', () => {
    withTempWorkspace((root) => {
      const sourcePath = path.join(root, '6-raw', 'inbox', 'malformed-attachment.md');
      fs.writeFileSync(sourcePath, 'malformed attachment', 'utf8');
      ...
      record.parent_source_id = source.source_id;
  ...
  it('rejects a record whose embedded ID differs from its requested filename and protects the canonical record', () => {
    withTempWorkspace((root) => {
      const parentPath = path.join(root, '6-raw', 'inbox', 'parent.md');
      const otherPath = path.join(root, '6-raw', 'inbox', 'other.md');
      const attachmentPath = path.join(root, '6-raw', 'inbox', 'export.csv');
      fs.writeFileSync(parentPath, 'parent', 'utf8');
      ...
      const parent = registerSource(root, { path: parentPath, sourceType: 'note' });
      const other = registerSource(root, { path: otherPath, sourceType: 'note' });
      const parentRecordPath = path.join(root, parent.record_path);
      const otherRecordPath = path.join(root, other.record_path);
      const otherBytesBefore = fs.readFileSync(otherRecordPath);
      fs.copyFileSync(otherRecordPath, parentRecordPath);

      assert.throws(
        () => readSourceRecord(root, parent.source_id),
        (error) => error instanceof SourceRegistryError
          && error.code === 'RECORD_FILENAME_CONFLICT'
          && error.details.requested_source_id === parent.source_id
          && error.details.embedded_source_id === other.source_id
      );
      assert.throws(
        () => registerAttachment(root, parent.source_id, attachmentPath, { label: 'export.csv' }),
        (error) => error instanceof SourceRegistryError && error.code === 'RECORD_FILENAME_CONFLICT'
      );

      assert.deepEqual(fs.readFileSync(otherRecordPath), otherBytesBefore);
      assert.deepEqual(readSourceRecord(root, other.source_id), other.record);
    });
  });
